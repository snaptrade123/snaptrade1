    ...
    <div className="border border-border rounded-md p-4 mb-4">
      ...
      <div className="relative h-[200px] w-full">
        {prices.map((price, index) => {
          ...
          const position = ((price.value! - paddedMin) / paddedRange) * 100;
          
          return (
            <div 
              key={price.type} 
              className={`absolute w-full left-0 trading-level trading-level-${price.type}`}
              style={{ bottom: `${position}%` }}
            >
              <div className="flex justify-between items-center px-2">
                <span className="text-xs font-medium">{price.label}</span>
                <span className={`text-sm font-bold ${
                  price.type === 'tp' ? 'text-emerald-500' : 
                  price.type === 'sl' ? 'text-red-500' : 
                  'text-primary'
                }`}>
